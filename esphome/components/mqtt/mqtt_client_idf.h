...
class MqttIdfClient : public MqttClientBase {
 public:
  ...
  void set_credentials(const char *username, const char *password = nullptr) override {
    if (username)
      this->username_ = username;
    if (password)
      this->password_ = password;
  }
  void set_will(const char *topic, uint8_t qos, bool retain, const char *payload = nullptr,
