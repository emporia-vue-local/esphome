...
def validate_phases(val):
    base_validated = cv.Schema(cv.ensure_list(
        {
    ...
    for i, phase in enumerate(base_validated):
        ...
        if input_wire == 'BLACK':
            if CONF_PHASE_ANGLE in phase:
                raise cv.Invalid("Phase angle is not supported for the black wire, "
                                 "only for the red and blue wires",
                                 path=[i, CONF_PHASE_ANGLE])
        elif input_wire == 'RED' or input_wire == 'BLUE':
            if CONF_FREQUENCY in phase:
                raise cv.Invalid("Frequency is not supported for the red and blue "
                                 "wires, only for the black wire",
